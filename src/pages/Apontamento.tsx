...
import { materialsData, type Material } from "@/data/MaterialsData";
import { tipoParadaValues, type TipoParada, getAreasByTipoParada, getEquipamentosByArea, getMotivosByArea } from "@/data/downtime";
// Helpers para tempo e formatação
const HOURS = Array.from({ length: 24 }, (_, i) => String(i).padStart(2, "0"));
const MINUTES = Array.from({ length: 60 }, (_, i) => String(i).padStart(2, "0"));

const nfPtBR2 = new Intl.NumberFormat("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 });
const formatPtBR2 = (n?: number) => (n == null || Number.isNaN(n) ? "" : nfPtBR2.format(n));
const nfPtBR3 = new Intl.NumberFormat("pt-BR", { minimumFractionDigits: 3, maximumFractionDigits: 3 });
const formatPtBR3 = (n?: number) => (n == null || Number.isNaN(n) ? "" : nfPtBR3.format(n));
...
const MACHINES = ["EA44", "EA45", "EA46"] as const;

type MachineKey = (typeof MACHINES)[number];

// Catálogos simples para espelhar o layout da imagem
const FAMILIAS = ["Extrusados", "VINAGRETE", "CHURRASCO", "CAMARÃO", "COSTELA", "BACON", "MEXICANA", "PÃO DE ALHO"] as const;
const SHIFT_NUMS = ["1", "2", "3"] as const; // exibimos 1/2/3 e salvamos A/B/C
const MOTIVOS_PERDA = [
  "N/A"
] as const;
// Downtime data now comes from @/data/downtime.ts with cascade logic

// Componente de seleção de horário HH:MM
const TimeSelect = ({
  label,
  hh,
  mm,
...
// Card de cada máquina com os campos na ORDEM EXATA solicitada
const MachineCard = ({ machine }: { machine: MachineKey }) => {
  ...
  const diferenca = rendimento != null ? rendimento - 85 : undefined;

  // Eficiência baseada em tempo de operação e produção líquida: Rendimento x Disponibilidade
  ...
  // Catálogos dependentes
  const turnoNum = data.turno === 'A' ? '1' : data.turno === 'B' ? '2' : data.turno === 'C' ? '3' : '';
  const prods = useMemo(() => {
    if (!data.familia) return materialsData;
    
    const familia = data.familia.toUpperCase();
    return materialsData.filter((mat) => {
      const material = mat.Material.toUpperCase();
      
      // Mapeamento específico para cada família
      switch (familia) {
        case 'MEXICANA':
          return material.includes('MEXICANA') || material.includes('MEX');
        case 'VINAGRETE':
          return material.includes('VINAGRETE');
        case 'CHURRASCO':
          return material.includes('CHURRASCO') || material.includes('CHUR');
        case 'CAMARÃO':
          return material.includes('CAMARAO') || material.includes('CAMARÃO');
        case 'COSTELA':
          return material.includes('COSTELA');
        case 'BACON':
          return material.includes('BACON');
        case 'PÃO DE ALHO':
          return material.includes('PAO DE ALHO') || material.includes('PÃO DE ALHO');
        case 'EXTRUSADOS':
          return material.includes('TORCIDA'); // Todos os produtos são torcidas
        default:
          return material.includes(familia);
      }
    });
  }, [data.familia]);

  // Cascade logic for downtime dropdowns
  const availableAreas = useMemo(() => getAreasByTipoParada(data.tipoParada), [data.tipoParada]);
  const availableEquipamentos = useMemo(() => getEquipamentosByArea(data.downtimeArea), [data.downtimeArea]);
  const availableMotivos = useMemo(() => getMotivosByArea(data.downtimeArea), [data.downtimeArea]);
  ...
  return (
    <Card className="shadow-sm">
      <CardHeader>
        <CardTitle className="text-base font-semibold">{machine} Torcida</CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        {/* Topo: Família e Turno */}
        <div className="grid grid-cols-2 gap-2">
          <div className="space-y-2">
            <Label>Família</Label>
          ...
          <div className="space-y-2">
            <Label>Turno</Label>
            <Select value={turnoNum} onValueChange={(v) => {
        ...
        {/* Bolsas produzidas, Eficiência EA, Perdas e Lote */}
        <div className="grid grid-cols-4 gap-2">
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(e) => updateMachineData(machine, { bolsasProduzidas: Number(e.target.value) })}
              placeholder="0"
            />
          </div>
          <div className="space-y-2">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(e) => updateMachineData(machine, { filmePerdasKg: Number(e.target.value) })}
              placeholder="0,000"
            />
          </div>
          <div className="space-y-2">
            <Label>Lote</Label>
            <Select>
              <SelectTrigger><SelectValue placeholder="LS 000" /></SelectTrigger>
              <SelectContent className="max-h-64 z-50 bg-background border shadow-md">
                <SelectItem value="LS000">LS 000</SelectItem>
              </SelectContent>
            </Select>
          </div>
        </div>

        {/* Motivo perda, Ef.balança, Overpack, Tipo Parada */}
        <div className="space-y-2">
        ...
        <div className="grid grid-cols-3 gap-2">
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(e) => updateMachineData(machine, { eficienciaBalanca: Number(e.target.value) })}
              placeholder="0"
            />
          </div>
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(e) => updateMachineData(machine, { overpackG: Number(e.target.value) })}
              placeholder="0"
            />
          </div>
          <div className="space-y-2">
            <Label>Tipo Parada</Label>
            <Select value={data.tipoParada || ""} onValueChange={(v) => {
              // Reset dependent fields when tipo parada changes
              updateMachineData(machine, { 
                tipoParada: v as TipoParada, 
                downtimeArea: "", 
                downtimeEquipamento: "", 
                downtimeMotivo: "" 
              });
        ...
        <div className="grid grid-cols-3 gap-2">
          ...
          <div className="space-y-2">
            ...
            <Input readOnly value={downtimeMin.toString()} />
          </div>
          <div className="space-y-2">
            <Label>Eficiência DWT</Label>
            <Input readOnly value={eficienciaDwt ? `${eficienciaDwt.toFixed(1)}%` : "0%"} />
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2 items-end">
          <div className="space-y-2">
            ...
            <Select 
              ...
              disabled={!data.tipoParada}
            >
              <SelectTrigger><SelectValue placeholder={data.tipoParada ? "Selecione" : "Selecione Tipo Parada primeiro"} /></SelectTrigger>
              <SelectContent className="max-h-64 z-50 bg-background border shadow-md">
        ...
        <div className="space-y-2">
          ...
          <Select 
            value={data.downtimeEquipamento || ""} 
            onValueChange={(v) => updateMachineData(machine, { downtimeEquipamento: v })}
            disabled={!data.downtimeArea}
          >
            <SelectTrigger><SelectValue placeholder={data.downtimeArea ? "Selecione" : "Selecione Área primeiro"} /></SelectTrigger>
            <SelectContent className="max-h-64 z-50 bg-background border shadow-md">
              {availableEquipamentos.map((a) => (
                <SelectItem key={a} value={a}>{a}</SelectItem>
              ))}
            </SelectContent>
          </Select>
        ...
        <div className="space-y-2">
          ...
          <Select 
            value={data.downtimeMotivo || ""} 
            onValueChange={(v) => updateMachineData(machine, { downtimeMotivo: v })}
            disabled={!data.downtimeArea}
          >
            <SelectTrigger><SelectValue placeholder={data.downtimeArea ? "Selecione" : "Selecione Área primeiro"} /></SelectTrigger>
            <SelectContent className="max-h-64 z-50 bg-background border shadow-md">
...
const Apontamento = () => {
  const scrollToPhotos = () => {
    const first = document.getElementById(`photos-${MACHINES[0]}`);
  ...
  return (
    <>
      <Helmet>
        <title>Apontamento de Produção - 3 Máquinas</title>
        <meta name="description" content="Lançamento de produção com 3 colunas por máquina, cálculos em tempo real e fotos." />
        <link rel="canonical" href="/apontamento" />
      </Helmet>
      <Header />
      <main className="container mx-auto px-4 py-6 space-y-6">
        {/* Topo com botões Home e Photo */}
        ...
        {/* Grid responsivo: desktop 3 colunas, mobile empilhado */}
